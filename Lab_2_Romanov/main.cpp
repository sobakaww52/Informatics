...
int main() {
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                if (event.mouseButton.button == sf::Mouse::Left) {
                    ...
                    float mathX = (mousePos.x - 400) / 45.0f; // Масштаб 30 по X
                    float mathY = -(mousePos.y - 300) / 45.0f; // Масштаб 100 по Y
        ...
        drawGraph(window, [](float x) { return (-0.1 * x * x) + 4;}, -10, 10, 45, 45, sf::Color::Blue);

        // 5 _   Отрисовка графика y2 = x * x - 5 (Замените на ваш график)
        drawGraph(window, [](float x) { return abs(x);}, -10, 10, 45, 45, sf::Color::Red);
